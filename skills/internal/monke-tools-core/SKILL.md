...
- `mt swing [target] [--codex]`: navigate to an existing session worktree, source checkout, previous Swing target, or same-repo pull request target; use `--codex` when running from Codex.
