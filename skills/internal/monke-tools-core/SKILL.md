...
- `mt cleanup`: clean eligible retained Sessions across all Roots, including partially removed Sessions and those awaiting finalization. Preview without changes with `--dry-run`; both modes accept `--json` and `--eligible` (hide skipped Sessions in human output) and work outside a repository. Every member must pass eligibility; unowned worktrees stay untouched. Dev servers and other process trees started from a worktree are stopped before removal once a day old; any process under a day old skips the Session and is listed.
