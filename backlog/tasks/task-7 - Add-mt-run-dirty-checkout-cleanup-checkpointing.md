---
id: TASK-7
title: Add mt work dirty-checkout cleanup checkpointing
status: Done
assignee:
  - '@codex'
...
Implement GitHub issue #14 from PRD #12 by extending mt work so dirty startup checkouts are checkpointed through the shared harness only when needed, the streamed Codex-backed implementer and reviewer workflow proceeds only after cleanup succeeds, and any commit outside the cleanup checkpoint phase is blocked and reported.
<!-- SECTION:DESCRIPTION:END -->

## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 mt work detects whether the startup checkout is dirty and only invokes cleanup when staged, unstaged, or untracked changes are present before continuing to the implementer and reviewer workflow
...
Completed the cleanup enforcement slice so the shipped `mt work` workflow safely handles dirty repos before the implementer and reviewer passes continue.
...
- After successful cleanup, `mt work` still runs the Codex-backed implementer and reviewer passes with live streaming and reports the combined workflow outcome at the end.
