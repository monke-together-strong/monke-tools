# Repo-specific Backlog.md notes

This repository is initialized for Backlog.md in CLI mode.

- Use `bunx backlog.md ...` for Backlog.md commands in this repo.
- Do not assume plain `backlog ...` is safe here; this machine already has a different `backlog` executable on `PATH`.
- When the generated guide below says `backlog ...`, translate it to `bunx backlog.md ...` unless you have explicitly verified that `backlog` resolves to Backlog.md.
- Repo usage notes and examples live in [docs/backlog-usage.md](docs/backlog-usage.md).
- `CLAUDE.md` is a symlink to this file so the same instructions are shared across agent clients.
